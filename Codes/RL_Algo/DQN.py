...
class DQN(RLAlgorithm):
    def __init__(self, input_shape, epochs=10, neurons_num:list = [128,32], lr=0.01, gamma=0.9,
                 epsilon=0.99, decay=0.000002, min_epsilon=0.0001, batch_size=32):
    ...
    def create_model(self):
        ...
        model.add(Dense(NUMBER_OF_ACTION, activation="softmax", name="Actions")) # sigmoid
    ...
    def replay(self, memory, callbacks_list, model_info_path, model_name):
        ...
        self.model.fit(np.squeeze(x_batch), np.squeeze(y_batch), epochs=self.epochs, batch_size=len(x_batch), validation_split=0.20,callbacks=callbacks_list, verbose=0)
